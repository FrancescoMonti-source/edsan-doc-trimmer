...
- **Output artifact**: The primary deliverable of this repository is an exported ONNX model (`model.onnx`) and tokenizer metadata packaged into `edsan-doc-trimmer-v1.1.0.zip`, executed by `redsan` via a background Python batch worker (`trim_batch_service.py`).
