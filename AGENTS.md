...
- **Output artifact**: The primary deliverable of this repository is an exported ONNX model (`model.onnx`) and tokenizer metadata packaged into `edsan-doc-trimmer-v1.3.0.zip`, executed by `redsan` via a background Python batch worker (`trim_batch_service.py`). Runtime inference uses ONNX Runtime providers; PyTorch is used for training and export.
