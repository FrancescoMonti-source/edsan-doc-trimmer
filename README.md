...
Part of the **redsan** ecosystem: trains student models (using `DrBERT/DrBERT-7GB`) with weak supervision from LLM teachers, exporting lightweight ONNX artifacts for high-throughput batch inference in `redsan` and EDSAN hospital pipelines.
...
4. **ONNX Export**: Exports the trained weights to an ONNX graph consumed upstream by `redsan::trim_doceds_onnx()`.
